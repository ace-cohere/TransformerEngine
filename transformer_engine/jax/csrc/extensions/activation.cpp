...
Error_Type ActLuFFI(cudaStream_t stream, Buffer_Type input_buf, Buffer_Type scale_buf,
  ...
  auto input_shape = std::vector<size_t>{m, static_cast<unsigned long>(act_len * n)};
  auto output_shape = std::vector<size_t>{m, static_cast<unsigned long>(n)};
  auto output_trans_shape = std::vector<size_t>{static_cast<unsigned long>(n), m};
...
Error_Type DActLuDBiasQuantizeFFI(cudaStream_t stream, Buffer_Type input_buf,
  ...
  auto input_shape = std::vector<size_t>{m, static_cast<unsigned long>(n)};
  auto act_input_shape = std::vector<size_t>{m, static_cast<unsigned long>(n * act_len)};
  auto output_shape = std::vector<size_t>{m, static_cast<unsigned long>(n * act_len)};
  auto output_trans_shape = std::vector<size_t>{static_cast<unsigned long>(n * act_len), m};
  auto dbias_shape = std::vector<size_t>{static_cast<unsigned long>(n * act_len)};
